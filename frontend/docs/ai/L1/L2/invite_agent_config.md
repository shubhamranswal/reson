...
| ------------------- | ---------------------------------------------------- | --------------------------------------------------------- |
| `SYSTEM_PROMPT`        | Long-form instructions for "Ada", an Agora assistant | The system prompt for the LLM.                            |
| `GREETING`          | Friendly first line                                   | Spoken on session start.                                 |

Agent behavior is configured in code: edit `GREETING` or `SYSTEM_PROMPT` directly.
...
Edit `SYSTEM_PROMPT`. Keep it under the LLM's context window — `gpt-4o-mini` handles thousands of tokens but very long prompts amplify latency.
