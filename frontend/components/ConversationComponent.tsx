...
export default function ConversationComponent({
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        console.log('[RESON DEBUG] AgoraVoiceAI.init START');
    ...
    return () => {
      console.log('[RESON DEBUG] AgoraVoiceAI CLEANUP', {
        isReady,
        joinSuccess,
        connectionState,
        isAgentConnected,
      ...
      try {
        ...
        if (ai) {
          console.log('[RESON DEBUG] Destroying AgoraVoiceAI instance');
          ai.unsubscribe();
          ai.destroy();
        }
      } catch (error) {
        console.error('[RESON DEBUG] Cleanup error:', error);
      }
    };
  ...
  useClientEvent(client, 'user-left', (user) => {
    if (user.uid.toString() === agentUID) {
      console.log('[RESON DEBUG] AGENT LEFT:', user.uid);
